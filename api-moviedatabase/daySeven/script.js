...
const getPopularMovies = async () => {
    ...
    .then(response => response.json())
        .then(response => {
            ...
            let showFavoriteMovies = false;
            showMovies(arrayMovies, showFavoriteMovies);
    })
    .catch(err => console.error(err));
}

const showMovies = async (arrayMovies, showFavoriteMovies) => {
...
const getFavoritesMovies = () => {
    ...
    let showFavoriteMovies = true;
    showMovies(moviesLocalStorage, showFavoriteMovies);
}
...
// ao pesquisar, retornar para lista inicial, se input for vazio
const changeSearchMovie = e => {
    let movieSearched = e.target.value;
    if (movieSearched && movieSearched.length > 0) {
        getMoviesSearch();
    } else {
        let containerMovies = document.querySelector(".main-container-descricao-filmes");
        containerMovies.innerHTML = "";
        getPopularMovies();
    }
}

// TODO: Organizar pastas, criando service para chamadas API e arquivo JS para localStorage